...
class NewmotorSpider(scrapy.Spider):
    name = "newmotor"
    custom_settings = {"FEEDS": {"data/motor.csv": {"format": "csv"}}}

    def start_requests(self):
        yield scrapy.Request(
            url="https://vehiculos.tucarro.com.co/motos_ITEM*CONDITION_2230284_NoIndex_True",
            callback=self.parse,
            meta={"offset": 1},
        )
    ...
    def parse(self, response):
        ...
        if next:
            yield scrapy.Request(
                url=f"https://vehiculos.tucarro.com.co/motos_Desde_{offset + 48}_ITEM*CONDITION_2230284_NoIndex_True",
                callback=self.parse,
